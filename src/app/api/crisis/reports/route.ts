  ...
  withAuth(async (req: AuthenticatedRequest) => {
    try {
      ...
      const filters: ReportFilters = {
        severityLevel: searchParams.get('severityLevel')?.split(',').map(Number) as CrisisSeverity[] || undefined,
...
export const POST = withRateLimit(5, 60000)(
  withAuth(async (req: AuthenticatedRequest) => {
    try {
      ...
      const data: CreateReportRequest = validation.data;
...
export const PUT = withRateLimit(30, 60000)(
  withCrisisCounselor(async (req: AuthenticatedRequest) => {
    try {
      ...
      const data: UpdateReportRequest = validation.data;
      ...
      notifyCounselors(CrisisEvents.REPORT_UPDATED, {
