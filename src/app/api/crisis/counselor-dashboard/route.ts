...
export const GET = withRateLimit(30, 60000)(
  withCrisisCounselor(async (req: AuthenticatedRequest) => {
    try {
      ...
      // Transform alerts
      const alertResponses: AlertResponse[] = recentAlerts.map(alert => ({
        id: alert.id,
        type: alert.type,
      ...
      const myAlertResponses: AlertResponse[] = myAlerts.map(alert => ({
        id: alert.id,
        type: alert.type,
        severity: alert.severity,
        userId: alert.userId,
